...
export function PortalStatusCard({ patientId, patientName, onStatusChange }: PortalStatusCardProps) {
    ...
    try {
      ...
      if (portalStatus && portalStatus.nextResendTime) {
        const secondsUntil = Math.max(
          0,
          Math.floor((portalStatus.nextResendTime.getTime() - Date.now()) / 1000)
        )
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">Online medical records and appointments</p>
          </div>
        </div>
        {getStatusBadge()}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {status.inviteCount > 0 && (
            <div className="flex items-center space-x-2 p-3 bg-gray-50 rounded-lg">
              ...
              <div>
                <p className="text-sm font-medium text-gray-700">
                  {status.inviteCount} {status.inviteCount === 1 ? 'invitation' : 'invitations'}
                </p>
        ...
        {status.enabled && status.lastInviteSent && (
          <div className="border-t pt-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-700">Last Invitation</span>
            ...
            >
              {sending ? (
                ...
                <>
                  ...
                  <span>{status.inviteCount > 0 ? 'Resend' : 'Send'} Portal Invitation</span>
                </>
              )}
            </button>
            {!status.canResend && countdown === 0 && (
        ...
        {!status.enabled && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p className="text-sm text-blue-800">
              Enable portal access to allow {patientName.split(' ')[0]} to view medical records,
              schedule appointments, and communicate securely online.
            </p>
          </div>
        )}
      </div>
