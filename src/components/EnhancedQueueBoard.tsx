...
export function EnhancedQueueBoard() {
  ...
  const patientMap = new Map<string, Patient>(
    patients.map((p) => [p.id, p] as [string, Patient]),
  );
  ...
  React.useEffect(() => {
    ...
    for (const stage of STAGES) {
      ...
      const waiting = stageItems.filter(
        (item) => item.status === "waiting",
      ).length;
      const inProgress = stageItems.filter(
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-gray-600">
            Real-time patient flow with clear handoffs
          </p>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="text-2xl font-bold">
                {metrics.waiting + metrics.inProgress}
              </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-sm text-gray-500">
                      {isNext
                        ? "Next"
                        : `~${index * (selectedMetrics?.avgTimeMinutes || 4)}m`}
                    </div>
                  </div>
                );
              })}
