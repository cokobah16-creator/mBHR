...
import type { ConflictField, PHISensitivity, ConflictPriority } from '@/services/conflictQueue'
...
const PHI_LABELS: Record<PHISensitivity, { label: string, color: string, icon: typeof ShieldExclamationIcon }> = {
  high: { label: 'High PHI', color: 'text-red-600 bg-red-50 border-red-200', icon: ShieldExclamationIcon },
  medium: { label: 'Medium PHI', color: 'text-amber-600 bg-amber-50 border-amber-200', icon: ExclamationTriangleIcon },
  low: { label: 'Low PHI', color: 'text-blue-600 bg-blue-50 border-blue-200', icon: ShieldCheckIcon },
  none: { label: 'Non-PHI', color: 'text-gray-600 bg-gray-50 border-gray-200', icon: CheckCircleIcon }
}

const PRIORITY_LABELS: Record<ConflictPriority, { label: string, color: string }> = {
      ...
      <div className="p-4 border-b bg-gray-50">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <span className={`px-2.5 py-1 rounded-full text-xs font-semibold border ${PRIORITY_LABELS[priority].color}`}>
              {PRIORITY_LABELS[priority].label} Priority
            </span>
            <PHIBadge sensitivity={phiSensitivity} />
          </div>
          <span className="text-sm text-gray-500">
            {differentFields.length} field{differentFields.length !== 1 ? 's' : ''} differ
          </span>
        </div>
        ...
        {(localTimestamp || remoteTimestamp) && (
          <div className="flex items-center gap-6 text-sm text-gray-600">
            ...
            {remoteTimestamp && (
              <div className="flex items-center gap-1">
                <ClockIcon className="h-4 w-4" />
                <span>Remote: {new Date(remoteTimestamp).toLocaleString()}</span>
              </div>
            )}
          </div>
        )}
          ...
          <div className="mt-3 p-3 bg-white rounded-lg border">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={`text-lg font-bold ${
                matchScore >= 0.8 ? 'text-red-600' :
                matchScore >= 0.6 ? 'text-amber-600' : 'text-gray-600'
              }`}>
            ...
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                className={`h-2 rounded-full ${
                  matchScore >= 0.8 ? 'bg-red-500' :
                  matchScore >= 0.6 ? 'bg-amber-500' : 'bg-gray-400'
                }`}
          ...
          >
            {showAllFields ? (
              ...
              <>
                ...
                Show {differentFields.length - 5} more field{differentFields.length - 5 !== 1 ? 's' : ''}
              </>
            )}
          </button>
        )}
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-red-600 mt-1">
                  This conflict involves highly sensitive patient information. Resolution will require
                  approval from an administrator before being finalized.
                </p>
              </div>
            </div>
          </div>
