...
export function QuestBoard({ onStartQuest }: QuestBoardProps) {
  ...
  const [activeTab, setActiveTab] = useState<"daily" | "weekly" | "event">(
    "daily",
  );
  ...
  const getTaskStatus = (task: GameTask) => {
    ...
    const completedToday = taskAttempts.filter(
  ...
  const getStatusDisplay = (task: GameTask, status: string) => {
    ...
    switch (status) {
      ...
      case "daily_limit":
        return {
          text: `Completed (${completedToday}/${task.maxPerDay})`,
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-gray-600">
              Complete tasks to earn tokens and badges
            </p>
          </div>
        </div>
