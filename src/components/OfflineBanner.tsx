...
export function OfflineBanner() {
  ...
  const shouldShow = !dismissed && (!isOnline || pendingCount > 0 || status === 'syncing' || status === 'error')
  ...
  const getBannerConfig = () => {
    if (!isOnline) {
      return {
        ...
        detail: pendingCount > 0 ? `${pendingCount} change${pendingCount === 1 ? '' : 's'} will sync when back online` : 'Changes will be saved locally',
