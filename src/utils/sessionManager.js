...
export class SessionManager {
    constructor(userType, onWarning, onExpire) {
    ...
    loadSessionTimes() {
        const savedActivity = localStorage.getItem(`${STORAGE_KEY_PREFIX}last_activity`);
        const savedCreation = localStorage.getItem(`${STORAGE_KEY_PREFIX}created_at`);
        if (savedActivity) {
        ...
        if (savedCreation) {
            ...
            this.sessionCreatedAt = Date.now();
            localStorage.setItem(`${STORAGE_KEY_PREFIX}created_at`, this.sessionCreatedAt.toString());
        }
    }
    ...
    handleActivity() {
        this.lastActivityTime = Date.now();
        localStorage.setItem(`${STORAGE_KEY_PREFIX}last_activity`, this.lastActivityTime.toString());
    }
    startActivityCheck() {
    ...
    checkSessionStatus() {
        ...
        if (timeSinceCreation >= maxDurationMs) {
            logger.info(`[SessionManager] Maximum session duration reached for ${this.userType}`);
 ...
 */
export async function validateAndRefreshPatientSession(sessionToken) {
    try {
