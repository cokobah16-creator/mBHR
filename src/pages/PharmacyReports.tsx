...
export default function PharmacyReports() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'dispensing' | 'expiry' | 'stock'>('overview')
  ...
  const dispensingStats: DispensingStats = useMemo(() => {
    const uniquePatients = new Set(filteredDispenses.map(d => d.patientId)).size
    const medicationCounts: Record<string, number> = {}

    filteredDispenses.forEach(d => {
      medicationCounts[d.itemName] = (medicationCounts[d.itemName] || 0) + d.qty
    })
    ...
    const days = dateRange === '7d' ? 7 : dateRange === '30d' ? 30 : dateRange === '90d' ? 90 :
      Math.max(1, Math.ceil((Date.now() - Math.min(...dispenses.map(d => new Date(d.dispensedAt).getTime()))) / (24 * 60 * 60 * 1000)))
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-600 text-sm">Analytics and insights for pharmacy operations</p>
          </div>
        </div>
        ...
        <div className="space-y-6">
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {lowStockItems.slice(0, 5).map(item => (
                  <span key={item.id} className="px-2 py-1 bg-red-100 text-red-800 rounded text-sm">
          ...
          {expiryAlerts.filter(a => a.daysUntilExpiry <= 30).length > 0 && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {expiryAlerts.filter(a => a.daysUntilExpiry <= 30).slice(0, 5).map(alert => (
                  <span
                    key={alert.batchId}
                    className={`px-2 py-1 rounded text-sm ${
                      alert.daysUntilExpiry <= 7 ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800'
                    }`}
                  >
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <h3 className="font-semibold text-gray-900 mb-4">Top Medications Dispensed</h3>
            {dispensingStats.topMedications.length === 0 ? (
              <p className="text-gray-500 text-center py-4">No dispensing data available</p>
            ) : (
                  ...
                  <div key={med.name} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-gray-600">{med.count.toLocaleString()} units</span>
                      </div>
                      <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-blue-600 rounded-full"
                          style={{ width: `${(med.count / dispensingStats.topMedications[0].count) * 100}%` }}
                        />
                      </div>
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredDispenses.slice(0, 50).map(d => (
                    <tr key={d.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm font-medium text-gray-900">{d.itemName}</td>
                      <td className="px-4 py-3 text-sm text-gray-600">{d.qty}</td>
                      <td className="px-4 py-3 text-sm text-gray-600">{d.dosage}</td>
            ...
            {filteredDispenses.length > 50 && (
              <div className="px-4 py-3 bg-gray-50 text-sm text-gray-600 text-center">
                Showing 50 of {filteredDispenses.length} records. Export CSV for full data.
              </div>
            )}
          </div>
        </div>
                      ...
                      <tr key={alert.batchId} className="hover:bg-gray-50">
                        <td className="px-4 py-3">
                          <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${
                            alert.daysUntilExpiry <= 0 ? 'bg-gray-100 text-gray-800' :
                            alert.daysUntilExpiry <= 7 ? 'bg-red-100 text-red-800' :
                            alert.daysUntilExpiry <= 30 ? 'bg-amber-100 text-amber-800' :
                            'bg-green-100 text-green-800'
                          }`}>
                            {alert.daysUntilExpiry <= 0 ? 'Expired' :
                             alert.daysUntilExpiry <= 7 ? 'Critical' :
                             alert.daysUntilExpiry <= 30 ? 'Warning' : 'OK'}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-sm font-medium text-gray-900">{alert.itemName}</td>
                        <td className="px-4 py-3 text-sm text-gray-600">{alert.lotNumber}</td>
                        <td className="px-4 py-3 text-sm text-gray-600">
                          {alert.expiryDate.toLocaleDateString()}
                        </td>
                        <td className="px-4 py-3 text-sm text-gray-600">{alert.quantity}</td>
                        <td className="px-4 py-3 text-sm font-medium">
                          <span className={
                            alert.daysUntilExpiry <= 0 ? 'text-gray-600' :
                            alert.daysUntilExpiry <= 7 ? 'text-red-600' :
                            alert.daysUntilExpiry <= 30 ? 'text-amber-600' : 'text-green-600'
                          }>
            ...
            <button
              onClick={() => exportToCSV(
                inventory.map(i => ({
                  ...
                  status: i.onHandQty <= i.reorderThreshold ? 'Low Stock' : 'In Stock',
                })),
                'stock_report'
              )}
              className="inline-flex items-center gap-2 px-3 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700"
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Reorder At</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Stock Level</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {inventory
                    .sort((a, b) => (a.onHandQty / a.reorderThreshold) - (b.onHandQty / b.reorderThreshold))
                    .map(item => {
                      const stockRatio = item.reorderThreshold > 0
                        ...
                        <tr key={item.id} className="hover:bg-gray-50">
                          <td className="px-4 py-3">
                            <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${
                              ...
                              {item.onHandQty <= item.reorderThreshold ? 'Low Stock' : 'In Stock'}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-sm font-medium text-gray-900">{item.itemName}</td>
                          <td className="px-4 py-3 text-sm text-gray-600">{item.unit}</td>
                          <td className="px-4 py-3 text-sm text-gray-900">{item.onHandQty}</td>
                          <td className="px-4 py-3 text-sm text-gray-600">{item.reorderThreshold}</td>
                          <td className="px-4 py-3">
                            <div className="w-24 h-2 bg-gray-100 rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full ${
                                  stockRatio <= 100 ? 'bg-red-500' :
                                  stockRatio <= 150 ? 'bg-amber-500' : 'bg-green-500'
                                }`}
