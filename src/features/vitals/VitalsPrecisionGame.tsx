...
export default function VitalsPrecisionGame() {
  ...
  const submitAnswer = async (
    ...
    const newAnswers = {
      ...userAnswers,
      [`${currentCase.id}_${metric}`]: answer,
    };
    ...
    const caseAnswers = caseMetrics.filter(
      (metric) => newAnswers[`${currentCase.id}_${metric}`],
    );
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-gray-600">
              Enhanced vital signs validation training
            </p>
          </div>
        </div>
            ...
            <div className="space-y-4 text-left max-w-md mx-auto mb-6">
              <div className="flex items-center space-x-3">
                ...
                <p className="text-gray-700">
                  Review {vitalCases.length} patient scenarios with age/sex
                  context
                </p>
              </div>
              <div className="flex items-center space-x-3">
                ...
                <p className="text-gray-700">
                  Identify normal vs abnormal vital signs using clinical ranges
                </p>
              </div>
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
                ...
                <p className="text-gray-700">
                  Earn tokens based on accuracy, speed, and streak performance
                </p>
              </div>
            </div>
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              Vitals Precision Complete!
            </h1>
            <p className="text-gray-600">
              Excellent work on clinical assessment!
            </p>
          </div>
        </div>
          ...
          <div className="text-center py-8">
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
              <p className="text-green-800 font-medium">
                🎉 Enhanced session completed! Waiting for supervisor approval
                to mint tokens.
              </p>
            </div>
  ...
  const caseMetrics = Object.keys(currentCase.vitals);
  const answeredMetrics = caseMetrics.filter(
    (metric) => userAnswers[`${currentCase.id}_${metric}`],
  );
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-gray-600">
              Case {caseIndex + 1} of {vitalCases.length}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-6">
          ...
          <div className="text-center flex items-center space-x-1">
            ...
            <div
              className={`text-2xl font-bold ${timeLeft <= 60 ? "text-red-600 animate-pulse" : "text-red-600"}`}
            >
              {formatTime(timeLeft)}
            </div>
          </div>
      ...
      <div className="card max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {Object.entries(currentCase.vitals).map(([metric, value]) => {
            ...
            const range =
              currentCase.normalRanges[
                metric as keyof typeof currentCase.normalRanges
              ];
            const isNormal = value >= range[0] && value <= range[1];
              ...
              >
                ...
                {answered ? (
                  <div className="flex items-center justify-center space-x-2">
                    ...
                    <span
                      className={`text-sm font-medium capitalize ${isCorrect ? "text-green-700" : "text-red-700"}`}
                    >
                      {answered} {isCorrect ? "✓" : "✗"}
                    </span>
                  </div>
        ...
        <div className="mt-6 text-center">
          ...
          {answeredMetrics.length === caseMetrics.length &&
            caseIndex < vitalCases.length - 1 && (
