  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      <div className="bg-white rounded-xl shadow-sm p-6">
        <h1 className="text-3xl font-bold text-gray-900">Prescription Refills</h1>
        <p className="text-gray-600 mt-2">Manage your prescriptions and request refills</p>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{refillRequests.filter(r => r.status === 'pending' || r.status === 'approved').length}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {refillRequests.filter(r => r.status !== 'dispensed').length > 0 && (
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 bg-blue-50">
            <h2 className="text-xl font-bold text-gray-900">Recent Refill Requests</h2>
          </div>

          <div className="divide-y divide-gray-200">
            {refillRequests.filter(r => r.status !== 'dispensed').map((request) => (
              <div key={request.id} className="p-6 hover:bg-gray-50 transition-colors">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3">
                      <h3 className="font-semibold text-gray-900">{request.medicationName}</h3>
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${getRequestStatusColor(request.status)}`}>
                    ...
                    <p className="text-gray-600">Pharmacy: {request.pharmacy}</p>
                    {request.status === 'ready' && (
                      <p className="text-green-600 font-medium mt-2">Ready for pickup!</p>
                    )}
                  </div>
                </div>
              </div>
                ...
                <div key={prescription.id} className="p-6 hover:bg-gray-50 transition-colors">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-gray-600 mt-2">{prescription.frequency}</p>
                      ...
                      <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        <div>
                          ...
                          <p className="font-semibold text-gray-900">{daysUntilRefill} days</p>
                        </div>
                        <div>
                          <p className="text-gray-500">Last filled</p>
                          <p className="font-semibold text-gray-900">{formatNigerianDate(prescription.lastDispensedDate)}</p>
                        </div>
                        <div>
                          <p className="text-gray-500">Refills remaining</p>
                          <p className="font-semibold text-gray-900">{prescription.refillsRemaining} of {prescription.totalRefills}</p>
                        </div>
                        <div>
                          <p className="text-gray-500">Days supply</p>
                          <p className="font-semibold text-gray-900">{prescription.daysSupply} days</p>
                        </div>
                      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-xl font-bold text-gray-900">All Active Prescriptions</h2>
        </div>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-semibold text-gray-900">{prescription.prescribedBy}</p>
                      </div>
                      <div>
                        <p className="text-gray-500">Last filled</p>
                        <p className="font-semibold text-gray-900">{formatNigerianDate(prescription.lastDispensedDate)}</p>
                    ...
                    {prescription.status !== 'expired' && prescription.refillsRemaining > 0 && (
                      <div className="mt-4">
                        <button
                          onClick={() => handleRequestRefill(prescription)}
      ...
      {showRefillModal && selectedPrescription && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl max-w-md w-full p-6">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">Request Refill</h2>

            <div className="bg-gray-50 rounded-lg p-4 mb-6">
              <p className="font-semibold text-gray-900 text-lg">
                {selectedPrescription.medicationName} {selectedPrescription.dosage}
              </p>
              <p className="text-gray-600 mt-1">{selectedPrescription.frequency}</p>
              <p className="text-sm text-gray-600 mt-2">
                Refills remaining: {selectedPrescription.refillsRemaining} of {selectedPrescription.totalRefills}
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-sm text-blue-800">
                  Your refill request will be reviewed by our pharmacy team. We'll notify you when it's ready for pickup.
                </p>
              </div>
