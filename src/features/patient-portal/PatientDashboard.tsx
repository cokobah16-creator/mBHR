...
const statusLabel: Record<VitalStatus, string> = {
  normal: "Normal", monitor: "Monitor", attention: "Attention",
};

function VitalCard({ label, value, unit, status }: { label: string; value: string; unit: string; status: VitalStatus }) {
  return (
...
function SupabaseDashboard() {
  ...
  const [state, setState] = useState<SupabaseDashboardState>({
    profile: null, vitals: [], medications: [], visits: [],
  });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatTile icon={<CalendarIcon className="w-6 h-6 text-blue-600" />} bg="bg-blue-100"
          label="Visits" value={visits.length} to="/patient/appointments" />
        <StatTile icon={<HeartIcon className="w-6 h-6 text-red-600" />} bg="bg-red-100"
          label="Medications" value={medications.length} to="/patient/medications" />
        <StatTile icon={<BeakerIcon className="w-6 h-6 text-purple-600" />} bg="bg-purple-100"
          label="Vital Records" value={vitals.length} to="/patient/medical-history" />
      </div>
...
function OfflineDashboard() {
  ...
  const { patient, upcomingAppointments, recentVitals, activeMedications } = data;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatTile icon={<CalendarIcon className="w-6 h-6 text-blue-600" />} bg="bg-blue-100"
          label="Upcoming Appointments" value={upcomingAppointments.length} to="/patient/appointments" />
        <StatTile icon={<EnvelopeIcon className="w-6 h-6 text-green-600" />} bg="bg-green-100"
          label="Unread Messages" value={data.unreadMessages} to="/patient/messages" />
        <StatTile icon={<BellIcon className="w-6 h-6 text-yellow-600" />} bg="bg-yellow-100"
          label="Notifications" value={data.unreadNotifications} to="/patient/notifications" />
      </div>

      {/* Vitals */}
      {recentVitals && (
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Recent Vitals</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {recentVitals.systolic && recentVitals.diastolic && (
              <VitalCard label="Blood Pressure"
                value={`${recentVitals.systolic}/${recentVitals.diastolic}`} unit="mmHg"
                status={bpStatus(recentVitals.systolic)} />
            )}
            {recentVitals.tempC && (
              <VitalCard label="Temperature" value={`${recentVitals.tempC}°C`} unit="celsius"
      ...
      {activeMedications.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Active Medications</h2>
          <div className="space-y-3">
  ...
  return (
    <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl shadow-sm p-6 text-white">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {[
          { to: "/patient/appointments", icon: <PlusIcon className="w-5 h-5" />, label: "Book Visit" },
          { to: "/patient/medical-history", icon: <ClipboardDocumentListIcon className="w-5 h-5" />, label: "View My Records" },
          { to: "/patient/messages", icon: <ChatBubbleLeftRightIcon className="w-5 h-5" />, label: "Talk to Health Worker" },
          { to: "/patient/medications", icon: <InformationCircleIcon className="w-5 h-5" />, label: "Get Medication Info" },
          { to: "/patient/outreach", icon: <MapPinIcon className="w-5 h-5" />, label: "Find Outreach Near Me" },
          { to: "/patient/export", icon: <ArrowDownTrayIcon className="w-5 h-5" />, label: "Download Records" },
          { to: "/patient/data-sharing", icon: <ShieldCheckIcon className="w-5 h-5" />, label: "Manage Data Sharing" },
        ].map(({ to, icon, label }) => (
          <Link key={to} to={to}
