...
export function PreVisitForms() {
  ...
  const { register, handleSubmit, formState: { errors }, reset } = useForm<PreVisitFormData>({
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <p className="text-gray-600 mt-2">Complete forms before your appointment to save time</p>
      </div>
              ...
              <div key={form.id} className="p-6 hover:bg-gray-50 transition-colors">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <h3 className="font-semibold text-gray-900 text-lg">{form.appointmentType}</h3>
                    <p className="text-gray-600 mt-1">
                      Appointment: {formatNigerianDate(form.appointmentDate)}
                    </p>
                    <p className="text-sm text-yellow-600 mt-2">Please complete before your visit</p>
                  </div>
                  <button
      ...
      {showFormModal && selectedForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white rounded-xl max-w-3xl w-full p-6 my-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Pre-Visit Medical Form</h2>
            <p className="text-gray-600 mb-6">
              {selectedForm.appointmentType} - {formatNigerianDate(selectedForm.appointmentDate)}
            </p>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div>
                ...
                {errors.chiefComplaint && (
                  <p className="text-red-600 text-sm mt-1">{errors.chiefComplaint.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                {errors.symptomDuration && (
                  <p className="text-red-600 text-sm mt-1">{errors.symptomDuration.message}</p>
                )}
              </div>
