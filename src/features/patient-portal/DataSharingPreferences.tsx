...
export function DataSharingPreferences({ patientId }: Props) {
  ...
  const getPurposeLabel = (purpose: string) => {
    const purposeInfo = TEFCA_EXCHANGE_PURPOSES[purpose as keyof typeof TEFCA_EXCHANGE_PURPOSES];
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Data Sharing Preferences</h2>
            <p className="text-gray-600">Control how your health data is shared</p>
          </div>
        </div>
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-amber-700 mt-1">
                These settings control how your health information may be shared through the
                Trusted Exchange Framework (TEFCA) with other healthcare organizations.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4 mb-6">
          ...
          <label className="flex items-start gap-3 p-4 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 transition-colors">
            <input
              ...
              onChange={(e) => setPreferences({ ...preferences, allow_ias_access: e.target.checked })}
              className="w-5 h-5 mt-0.5 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
            />
            <div>
              <span className="font-medium text-gray-900">Individual Access Services (IAS)</span>
              <p className="text-sm text-gray-600">
                Allow yourself to access and download your own health records through third-party apps.
                This is required for personal health record apps.
              </p>
            </div>
          </label>
          ...
          <label className="flex items-start gap-3 p-4 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 transition-colors">
            <input
              ...
              onChange={(e) => setPreferences({ ...preferences, allow_treatment_access: e.target.checked })}
              className="w-5 h-5 mt-0.5 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
            />
            <div>
              <span className="font-medium text-gray-900">Treatment</span>
              <p className="text-sm text-gray-600">
                Allow healthcare providers to access your records for treatment purposes.
                This helps doctors and hospitals provide better care.
              </p>
            </div>
          </label>
          ...
          <label className="flex items-start gap-3 p-4 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 transition-colors">
            <input
              ...
              onChange={(e) => setPreferences({ ...preferences, allow_payment_access: e.target.checked })}
              className="w-5 h-5 mt-0.5 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
            />
            <div>
              <span className="font-medium text-gray-900">Payment</span>
              <p className="text-sm text-gray-600">
                Allow health plans and clearinghouses to access records for payment activities.
                This may be needed for insurance claims.
              </p>
            </div>
          </label>
          ...
          <label className="flex items-start gap-3 p-4 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 transition-colors">
            <input
              ...
              onChange={(e) => setPreferences({ ...preferences, allow_operations_access: e.target.checked })}
              className="w-5 h-5 mt-0.5 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
            />
            <div>
              <span className="font-medium text-gray-900">Healthcare Operations</span>
              <p className="text-sm text-gray-600">
                Allow access for quality assessment, training, and other healthcare operations.
                Data is typically de-identified for these purposes.
              </p>
            </div>
          </label>
        </div>
        ...
        <div className="mb-6">
          <label className="flex items-center gap-3 p-4 bg-blue-50 rounded-lg cursor-pointer">
            <input
              ...
              onChange={(e) => setPreferences({ ...preferences, require_notification: e.target.checked })}
              className="w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
            />
            <div className="flex items-center gap-2">
              <BellIcon className="w-5 h-5 text-blue-600" />
              <span className="font-medium text-blue-900">Notify me when my data is accessed</span>
            </div>
          </label>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h3 className="font-semibold text-gray-900">Recent Data Access History</h3>
        </div>
            ...
            {accessLogs.map((log) => (
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    <p className="font-medium text-gray-900">{log.requesting_organization}</p>
                    <p className="text-sm text-gray-600">{getPurposeLabel(log.exchange_purpose)}</p>
                  </div>
