...
export default function PharmacyStock() {
  ...
  const [filterType, setFilterType] = useState<
    "all" | "low_stock" | "expired" | "expiring_soon"
  >("all");
  ...
  const handleDeleteItem = async (itemId: string) => {
    if (
      !confirm(
        "Are you sure you want to delete this medication? This will also delete all associated batches.",
      )
    ) {
      return;
    }
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600">
          Only pharmacists and administrators can access pharmacy stock.
        </p>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Pharmacy Stock</h1>
            <p className="text-gray-600">
              Manage pharmaceutical inventory and batches
            </p>
          </div>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="card bg-yellow-50 border-yellow-200">
          ...
          <div className="text-2xl font-bold text-yellow-800">
            {
              items.filter((item) => item.onHandQty <= item.reorderThreshold)
                .length
            }
          </div>
        </div>
      ...
      {showAddBatch && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">
              Add New Batch -{" "}
              {items.find((i) => i.id === showAddBatch)?.medName}
            </h2>
