...
export default function ApprovalInbox() {
  ...
  const rejectSession = async (sessionId: string) => {
    if (
      !confirm(
        "Are you sure you want to reject this session? This cannot be undone.",
      )
    ) {
      return;
    }
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600">
          Only administrators can approve game sessions.
        </p>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Approval Inbox</h1>
            <p className="text-gray-600">
              Review and approve completed game sessions
            </p>
          </div>
        </div>
        ...
        {pendingSessions.length > 0 && (
          ...
          >
            ...
            <span>
              {processing === "bulk"
                ? "Approving..."
                : `Approve All (${pendingSessions.length})`}
            </span>
          </button>
        )}
      </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  <div className="flex-shrink-0">
                    <span
                      className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getGameTypeColor(session.type)}`}
                    >
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-4 text-sm text-gray-600">
                      ...
                      <span>
                        Completed: {session.finishedAt.toLocaleTimeString()}
                      </span>
                    </div>
                  </div>
                </div>
