...
export const doctorService = {
  async createPatientFlag(flag: Omit<PatientFlag, 'id' | 'created_at' | 'status'>): Promise<PatientFlag | null> {
  ...
  async createReferral(referral: Omit<Referral, 'id' | 'created_at' | 'updated_at'>): Promise<Referral | null> {
    if (!supabase) throw new Error('Supabase not initialized')
  ...
  async getPrescriptionTemplates(org_id: string, condition?: string): Promise<PrescriptionTemplate[]> {
    if (!supabase) throw new Error('Supabase not initialized')
  ...
  async getSiteFormulary(site_id: string, searchTerm?: string): Promise<SiteFormulary[]> {
    if (!supabase) throw new Error('Supabase not initialized')
  ...
  async getDoctorAnalytics(event_id: string, doctor_id: string): Promise<DoctorAnalytics | null> {
    if (!supabase) throw new Error('Supabase not initialized')
