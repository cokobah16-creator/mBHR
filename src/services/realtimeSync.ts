...
class RealtimeSyncService {
  ...
  private async handleReconnect(channelKey: string, config: SubscriptionConfig): Promise<void> {
    ...
    logger.log(`Reconnecting to ${channelKey} in ${delay}ms (attempt ${this.reconnectAttempts})`)
  ...
  subscribeToVitals(patientId: string, callback: SubscriptionCallback): () => void {
  ...
  subscribeToConsultations(patientId: string, callback: SubscriptionCallback): () => void {
    return this.subscribe({
      table: 'consultations',
      filter: `patient_id=eq.${patientId}`,
      callback
  ...
  subscribeToAppointments(patientId: string, callback: SubscriptionCallback): () => void {
    return this.subscribe({
      table: 'appointments',
      filter: `patient_id=eq.${patientId}`,
      callback
  ...
  subscribeToMessages(patientId: string, callback: SubscriptionCallback): () => void {
    return this.subscribe({
  ...
  subscribeToNotifications(patientId: string, callback: SubscriptionCallback): () => void {
    return this.subscribe({
  ...
  subscribeToQueue(stage?: string, callback?: SubscriptionCallback): () => void {
