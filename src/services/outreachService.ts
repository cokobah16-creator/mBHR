...
export const outreachService = {
  ...
  async createOutreachEvent(event: Omit<OutreachEvent, 'id' | 'created_at' | 'updated_at'>): Promise<OutreachEvent | null> {
    if (!supabase) throw new Error('Supabase not initialized')
  ...
  async getEventStaffAssignments(event_id: string): Promise<EventStaffAssignment[]> {
    if (!supabase) throw new Error('Supabase not initialized')
  ...
  async getUserEventAssignments(user_id: string): Promise<EventStaffAssignment[]> {
    if (!supabase) throw new Error('Supabase not initialized')
  ...
  async setDefaultOrgSite(user_id: string, org_id: string, site_id?: string): Promise<boolean> {
    if (!supabase) throw new Error('Supabase not initialized')
