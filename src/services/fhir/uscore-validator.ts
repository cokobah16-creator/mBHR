...
import type { FHIRAllergyIntolerance, FHIRDiagnosticReport, FHIRCondition } from './dexieAdapters';
...
function addWarning(warnings: ValidationError[], path: string, message: string): void {
...
export function validateUSCorePatient(patient: FHIRPatient): ValidationResult {
  ...
  if (!patient.identifier || patient.identifier.length === 0) {
    addError(errors, 'Patient.identifier', 'At least one identifier is required');
  } else {
    const hasValidIdentifier = patient.identifier.some(id => id.system && id.value);
    if (!hasValidIdentifier) {
      addError(errors, 'Patient.identifier', 'Identifier must have both system and value');
    }
  }
  ...
  if (!patient.name || patient.name.length === 0) {
    ...
    const hasValidName = patient.name.some(n => n.family || (n.given && n.given.length > 0));
    if (!hasValidName) {
  ...
  if (!patient.gender) {
    ...
    addError(errors, 'Patient.gender', `Invalid gender value: ${patient.gender}`);
  }

  if (!patient.birthDate) {
    addWarning(warnings, 'Patient.birthDate', 'Birth date is recommended for US Core compliance');
  } else {
    const dateRegex = /^\d{4}(-\d{2}(-\d{2})?)?$/;
    if (!dateRegex.test(patient.birthDate)) {
      addError(errors, 'Patient.birthDate', 'Birth date must be in YYYY, YYYY-MM, or YYYY-MM-DD format');
    }
  }
...
export function validateUSCoreVitalSigns(observation: FHIRObservation): ValidationResult {
  const errors: ValidationError[] = [];
  const warnings: ValidationError[] = [];
  ...
  if (!observation.category || observation.category.length === 0) {
    ...
    if (!hasVitalSignsCategory) {
      addError(errors, 'Observation.category', 'Must include vital-signs category');
    }
  }
  ...
  if (!observation.effectiveDateTime) {
    addError(errors, 'Observation.effectiveDateTime', 'Effective date/time is required');
  }
...
export function validateUSCoreBloodPressure(observation: FHIRObservation): ValidationResult {
  const baseResult = validateUSCoreVitalSigns(observation);
  const errors = [...baseResult.errors];
  const warnings = [...baseResult.warnings];

  if (!observation.component || observation.component.length < 2) {
    addError(errors, 'Observation.component', 'Blood pressure must have systolic and diastolic components');
  } else {
    ...
    if (!hasSystolic) {
      addError(errors, 'Observation.component', 'Systolic component (LOINC 8480-6) is required');
    }
    if (!hasDiastolic) {
      addError(errors, 'Observation.component', 'Diastolic component (LOINC 8462-4) is required');
    }

    for (const component of observation.component) {
      if (!component.valueQuantity?.value) {
        addError(errors, 'Observation.component.valueQuantity', 'Component must have a numeric value');
      }
    }
  }
...
export function validateUSCoreMedicationRequest(medRequest: FHIRMedicationRequest): ValidationResult {
  const errors: ValidationError[] = [];
  const warnings: ValidationError[] = [];
  ...
  if (!medRequest.medicationCodeableConcept && !('medicationReference' in medRequest)) {
    addError(errors, 'MedicationRequest.medication[x]', 'Medication is required (either codeableConcept or reference)');
  } else if (medRequest.medicationCodeableConcept) {
    if (!medRequest.medicationCodeableConcept.text && !medRequest.medicationCodeableConcept.coding?.length) {
      addError(errors, 'MedicationRequest.medicationCodeableConcept', 'Medication must have text or coding');
    }
  }
  ...
  if (!medRequest.subject) {
    ...
    addError(errors, 'MedicationRequest.subject', 'Subject must have a reference');
  }

  if (!medRequest.authoredOn) {
    addWarning(warnings, 'MedicationRequest.authoredOn', 'AuthoredOn is recommended');
  }
...
export function validateUSCoreEncounter(encounter: FHIREncounter): ValidationResult {
  const errors: ValidationError[] = [];
  const warnings: ValidationError[] = [];

  if (!encounter.identifier || encounter.identifier.length === 0) {
    addWarning(warnings, 'Encounter.identifier', 'At least one identifier is recommended');
  }
...
export function validateUSCoreAllergyIntolerance(allergy: FHIRAllergyIntolerance): ValidationResult {
  const errors: ValidationError[] = [];
  const warnings: ValidationError[] = [];

  if (!allergy.clinicalStatus) {
    addError(errors, 'AllergyIntolerance.clinicalStatus', 'Clinical status is required');
  }

  if (!allergy.code) {
    addError(errors, 'AllergyIntolerance.code', 'Code is required');
  } else if (!allergy.code.text && !allergy.code.coding?.length) {
    addError(errors, 'AllergyIntolerance.code', 'Code must have text or coding');
  }

  if (!allergy.patient) {
    addError(errors, 'AllergyIntolerance.patient', 'Patient is required');
  } else if (!allergy.patient.reference) {
    addError(errors, 'AllergyIntolerance.patient', 'Patient must have a reference');
  }

  if (!allergy.verificationStatus) {
    addWarning(warnings, 'AllergyIntolerance.verificationStatus', 'Verification status is recommended');
  }
...
export function validateUSCoreCondition(condition: FHIRCondition): ValidationResult {
  const errors: ValidationError[] = [];
  const warnings: ValidationError[] = [];
  ...
  if (!condition.verificationStatus) {
    addWarning(warnings, 'Condition.verificationStatus', 'Verification status is recommended');
  }
...
export function validateUSCoreDiagnosticReport(report: FHIRDiagnosticReport): ValidationResult {
  const errors: ValidationError[] = [];
  const warnings: ValidationError[] = [];
  ...
  if (!report.subject) {
    ...
    addError(errors, 'DiagnosticReport.subject', 'Subject must have a reference');
  }

  if (!report.effectiveDateTime) {
    addWarning(warnings, 'DiagnosticReport.effective[x]', 'Effective date is recommended');
  }
...
export function validateResource(resource: FHIRResource): ValidationResult {
  switch (resource.resourceType) {
    ...
    case 'Observation': {
      ...
      const isBloodPressure = obs.code?.coding?.some(c => c.code === '85354-9');
      if (isBloodPressure) {
    ...
    case 'AllergyIntolerance':
      return validateUSCoreAllergyIntolerance(resource as FHIRAllergyIntolerance);
    case 'Condition':
    ...
    default:
      return {
        valid: true,
        errors: [],
        warnings: [{ path: resource.resourceType, message: 'No US Core validator available for this resource type', severity: 'warning' }],
        profile: 'unknown',
      };
  }
}
...
export function validateBundle(resources: FHIRResource[]): BundleValidationResult {
  ...
  for (const resource of resources) {
    ...
    const key = resource.id || `${resource.resourceType}-${Math.random().toString(36).substr(2, 9)}`;
