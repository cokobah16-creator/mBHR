...
import type { Patient, Vital, Consultation, Dispense, Visit, PatientAllergy, Meta } from '../../db';
...
import { validateBundle, type BundleValidationResult } from './uscore-validator';
...
async function logFHIRExport(auditLog: FHIRExportAuditLog): Promise<void> {
  try {
    ...
    const logs: FHIRExportAuditLog[] = (existingLogs?.value as FHIRExportAuditLog[]) || [];
...
function isWithinDateRange(date: Date | string | undefined, rangeStart: Date | null): boolean {
...
async function getLastExportTimestamp(patientId: string): Promise<string | undefined> {
  const meta = await db.meta.get(`${META_KEY_LAST_EXPORT}${patientId}`);
  return meta?.value as string | undefined;
}

async function setLastExportTimestamp(patientId: string, timestamp: string): Promise<void> {
  await db.meta.put({
...
export async function exportPatientEHI(
  ...
  try {
    ...
    if (includeVitals) {
      ...
      resourceCounts['Observation'] = (resourceCounts['Observation'] || 0) +
    ...
    if (includeAllergies) {
      const allergies = await db.patientAllergies.where('patientId').equals(patientId).toArray();
...
export async function getExportPreview(patientId: string): Promise<{
  ...
  const [vitals, dispenses, visits, consultations, allergies] = await Promise.all([
  ...
  const totalResources = 1 + observationCount + dispenses + visits + consultations + allergies;
...
export function downloadNDJSON(
  ndjson: string,
  filename: string
): void {
