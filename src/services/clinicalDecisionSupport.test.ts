...
describe("Clinical Decision Support - Vitals Analysis", () => {
  ...
  it("should identify severe hypoxemia", () => {
    ...
    expect(analysis.recommendations).toContain(
      "URGENT: Oxygen therapy required immediately",
    );
  });
...
describe("Clinical Decision Support - Medication Adherence", () => {
  ...
  it("should provide interventions for high-risk patients", () => {
    ...
    expect(score.interventions.some((i) => i.includes("medication"))).toBe(
      true,
    );
  });
});
...
describe("Clinical Decision Support - SOAP Suggestions", () => {
  ...
  it("should suggest questions for pain in subjective", () => {
    ...
    expect(suggestions.suggestions.some((s) => s.includes("Location"))).toBe(
      true,
    );
    expect(suggestions.suggestions.some((s) => s.includes("Severity"))).toBe(
      true,
    );
  });
  ...
  it("should suggest malaria treatment in plan", () => {
    ...
    expect(
      suggestions.suggestions.some(
        (s) => s.includes("ACT") || s.includes("Antimalarial"),
      ),
    ).toBe(true);
  });
  ...
  it("should suggest hypertension management in plan", () => {
    ...
    expect(
      suggestions.suggestions.some((s) =>
        s.toLowerCase().includes("antihypertensive"),
      ),
    ).toBe(true);
    expect(
      suggestions.suggestions.some((s) =>
        s.toLowerCase().includes("lifestyle"),
      ),
    ).toBe(true);
  });
...
describe("Clinical Decision Support - Clinical Alerts", () => {
  ...
  it("should create clinical alert", async () => {
    ...
    (db.clinicalAlerts.add as any).mockResolvedValue("alert-123");
  ...
  it("should generate alerts for critical vitals", async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (db.clinicalAlerts.add as any).mockResolvedValue("alert-456");
