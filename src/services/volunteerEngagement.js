...
class VolunteerEngagementSystem {
    ...
    async calculateBadges(userId, patientsHelped, hours, visits) {
        const badges = [];
        if (patientsHelped >= 1) {
            badges.push({
                id: 'first-patient',
    ...
    determineSpecializations(skills) {
    ...
    async submitVolunteerStory(story) {
        return {
            id: `story-${Date.now()}`,
            ...story,
            likes: 0,
    ...
    async getVolunteerStories(limit = 10) {
        return [
            {
                id: 'story-1',
                volunteerId: 'v-001',
    ...
    async calculateVolunteerROI(userId) {
        const profile = await this.getVolunteerProfile(userId);
        return {
