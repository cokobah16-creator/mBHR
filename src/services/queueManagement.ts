...
export class QueueManagement {
  ...
  async addToQueue(
    ...
    logger.log(
      `Added patient ${patientId} to ${stage} queue at position ${position} with priority ${priority}`,
    );
    return queueItem;
  }
  ...
  async skipQueue(patientId: string, reason: string = "urgent"): Promise<void> {
    ...
    logger.log(
      `Patient ${patientId} moved to front of ${item.stage} queue: ${reason}`,
    );
  }
  ...
  async getQueueStats(stage: QueueStage): Promise<QueueStats> {
    ...
    const completedToday = items.filter(
      (i) => i.status === "done" && epochDay(i.updatedAt) === today,
    );
  ...
  async checkStaleQueues(): Promise<void> {
    ...
    for (const item of staleItems) {
      logger.warn(
        `Stale queue item detected: Patient ${item.patientId} waiting ${Math.floor((now - item.updatedAt.getTime()) / 60000)} minutes`,
      );
