...
describe('Predictive Queue - Wait Time Predictions', () => {
    ...
    it('should predict wait times for all stages', async () => {
        ...
        mbhrDb.queue_metrics.get.mockResolvedValue({
            avgServiceSec: 300
        });
    ...
    it('should identify bottlenecks correctly', async () => {
        ...
        const predictions = await predictiveQueue.predictWaitTimes();
        const hasBottleneck = predictions.some(p => p.bottleneckRisk === 'high' || p.bottleneckRisk === 'critical');
...
describe('Predictive Queue - Queue Optimization', () => {
    ...
    it('should optimize queue order based on priority scores', async () => {
        ...
        db.patients.get.mockResolvedValue({
            id: 'p1',
            givenName: 'John',
            familyName: 'Doe',
            dob: '1950-01-01'
        });
        db.vitals.where.mockReturnValue({
            equals: vi.fn(() => ({
                reverse: vi.fn(() => ({
                    limit: vi.fn(() => ({
                        toArray: vi.fn().mockResolvedValue([
        ...
        db.patientAllergies.where.mockReturnValue({
            equals: vi.fn(() => ({
                and: vi.fn(() => ({
                    toArray: vi.fn().mockResolvedValue([])
                }))
    ...
    it('should prioritize patients with critical vitals', async () => {
        ...
        db.patients.get.mockResolvedValue(mockPatient);
        db.vitals.where.mockReturnValue({
            equals: vi.fn(() => ({
                reverse: vi.fn(() => ({
                    limit: vi.fn(() => ({
...
describe('Predictive Queue - Queue Metrics', () => {
    ...
    it('should calculate queue metrics', async () => {
        ...
        mbhrDb.tickets.where.mockReturnValue({
            equals: vi.fn(() => ({
                filter: vi.fn(() => ({
                    toArray: vi.fn().mockResolvedValue(Array(50).fill(null).map((_, i) => ({
                        id: `t${i}`,
                        currentStage: 'registration',
