 ...
 */
async function checkRateLimit(patient: Patient): Promise<{ allowed: boolean; message?: string; waitMs?: number }> {
 ...
 */
export async function getPortalStatus(patientId: string): Promise<PortalStatusInfo | null> {
  try {
    const patient = await db.patients.get(patientId)
...
export async function findEligiblePatients(filters: {
  ...
  try {
    ...
    return patients.filter(p => {
      ...
      if (filters.contactMethod === 'any' && !hasEmail && !hasPhone) return false
