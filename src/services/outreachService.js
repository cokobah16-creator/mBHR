...
export const outreachService = {
    ...
    async getOutreachEvent(event_id) {
        if (!supabase)
            throw new Error('Supabase not initialized');
        const { data, error } = await supabase
            .from('outreach_events')
    ...
    async createOutreachEvent(event) {
        if (!supabase)
            throw new Error('Supabase not initialized');
        const { data, error } = await supabase
            .from('outreach_events')
    ...
    async updateOutreachEvent(event_id, updates) {
        if (!supabase)
            throw new Error('Supabase not initialized');
    ...
    async getEventStaffAssignments(event_id) {
        if (!supabase)
            throw new Error('Supabase not initialized');
        const { data, error } = await supabase
    ...
    async assignStaffToEvent(assignment) {
        if (!supabase)
            throw new Error('Supabase not initialized');
        const { data, error } = await supabase
            .from('event_staff_assignments')
    ...
    async checkInStaff(assignment_id) {
        if (!supabase)
            throw new Error('Supabase not initialized');
        const { error } = await supabase
            .from('event_staff_assignments')
    ...
    async checkOutStaff(assignment_id) {
        if (!supabase)
            throw new Error('Supabase not initialized');
        const { error } = await supabase
            .from('event_staff_assignments')
    ...
    async getUserEventAssignments(user_id) {
        if (!supabase)
            throw new Error('Supabase not initialized');
        const { data, error } = await supabase
            .from('event_staff_assignments')
    ...
    async getActiveEventForUser(user_id) {
        if (!supabase)
            throw new Error('Supabase not initialized');
    ...
    async getUserDefaultOrgSite(user_id) {
        if (!supabase)
            throw new Error('Supabase not initialized');
        const { data, error } = await supabase
            .from('user_org_sites')
    ...
    async setDefaultOrgSite(user_id, org_id, site_id) {
        if (!supabase)
            throw new Error('Supabase not initialized');
        await supabase
            .from('user_org_sites')
