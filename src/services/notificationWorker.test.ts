    ...
    outboundMessages: {
      where: vi.fn((field: string) => ({
        equals: vi.fn((value: string) => ({
          ...
          count: vi.fn(() => {
            const count = Array.from(mockOutboundMessages.values())
              .filter(msg => msg[field] === value).length
  ...
  supabase: {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
          lte: vi.fn(() => ({
            limit: vi.fn(() => Promise.resolve({ data: mockMedicationReminders, error: null }))
          }))
        }))
      })),
      update: vi.fn(() => ({
...
describe('notificationWorker', () => {
  ...
  describe('startNotificationWorker/stopNotificationWorker', () => {
    it('should start and stop the worker', async () => {
      const { startNotificationWorker, stopNotificationWorker } = await import('./notificationWorker')
...
describe('SMS delivery logic', () => {
  ...
  it('should handle successful SMS delivery', async () => {
    ...
    const response = await fetch('https://test.supabase.co/functions/v1/send-sms-reminder', {
      method: 'POST',
