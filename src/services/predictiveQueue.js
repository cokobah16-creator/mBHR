...
class PredictiveQueueSystem {
    constructor() {
        this.HISTORICAL_DAYS = 30;
        this.PEAK_THRESHOLD = 1.5;
        this.BOTTLENECK_THRESHOLD = 15;
    }
    ...
    async predictStageWaitTime(stage) {
        ...
        const historicalVolume = historicalData.filter(d => new Date(d.timestamp).getHours() === currentHour &&
        ...
        const predictedWaitMinutes = Math.round((waiting * avgServiceTime * trendMultiplier) / 60);
        const nextPatientETA = new Date(Date.now() + (avgServiceTime * 1000));
        const bottleneckRisk = this.assessBottleneckRisk(waiting, inProgress, avgServiceTime, avgVolume);
        const recommendedActions = this.generateRecommendations(stage, waiting, predictedWaitMinutes, bottleneckRisk);
    ...
    assessBottleneckRisk(waiting, inProgress, avgServiceTime, avgVolume) {
        const waitMinutes = (waiting * avgServiceTime) / 60;
        const volumeRatio = waiting / Math.max(avgVolume, 1);
        if (waitMinutes > 60 || volumeRatio > 3)
            return 'critical';
        if (waitMinutes > 45 || volumeRatio > 2)
            return 'high';
        if (waitMinutes > 30 || volumeRatio > 1.5)
            return 'moderate';
        if (waitMinutes > 15 || volumeRatio > 1.2)
            return 'low';
        return 'none';
    }
    generateRecommendations(stage, waiting, predictedWait, bottleneckRisk) {
    ...
    calculateConfidence(historicalPoints, currentVolume) {
    ...
    async optimizeQueueOrder(stage) {
    ...
    async calculatePriorityScore(patient, queueItem) {
        let score = 50;
        ...
        if (recentVitals.length > 0) {
            ...
            if (vitals.tempC && vitals.tempC > 39.5)
                score += 30;
            else if (vitals.tempC && vitals.tempC > 38.5)
                score += 20;
            if (vitals.systolic && (vitals.systolic > 180 || vitals.systolic < 90))
                score += 30;
            else if (vitals.systolic && (vitals.systolic > 160 || vitals.systolic < 100))
                score += 20;
        ...
        const allergies = await db.patientAllergies
            ...
            .and(a => a.isActive === 1)
            .toArray();
        if (allergies.some(a => a.severity === 'life-threatening'))
            score += 15;
        if (patient.dob) {
    ...
    calculateIdealPosition(queue, priorityScore) {
        if (priorityScore >= 80)
            return 1;
        if (priorityScore >= 70)
            return Math.min(3, Math.ceil(queue.length * 0.2));
        if (priorityScore >= 60)
            return Math.min(5, Math.ceil(queue.length * 0.3));
        return Math.ceil(queue.length * (1 - priorityScore / 100));
    }
    explainPriorityReason(patient, score) {
        if (score >= 80)
            return 'Critical vital signs or severe symptoms requiring immediate attention';
        if (score >= 70)
            return 'High-risk patient with concerning vital signs';
        if (score >= 60)
            return 'Vulnerable population (elderly/pediatric) or extended wait time';
        if (score >= 55)
    ...
    calculateSatisfactionScore(avgWaitTime, avgServiceTime) {
        let score = 100;
        if (avgWaitTime > 60 * 60)
            score -= 40;
        else if (avgWaitTime > 45 * 60)
            score -= 30;
        else if (avgWaitTime > 30 * 60)
            score -= 20;
        else if (avgWaitTime > 15 * 60)
            score -= 10;
        if (avgServiceTime > 600)
            score -= 10;
        else if (avgServiceTime > 480)
            score -= 5;
        return Math.max(0, score);
    }
    async getWaitingCount(stage) {
    ...
    async getInProgressCount(stage) {
        const count1 = await db.queue
            .where('stage').equals(stage)
    ...
    async generateQueueReport(startDate, endDate) {
        const stages = ['registration', 'vitals', 'consult', 'pharmacy'];
        const stageMetrics = await Promise.all(stages.map(stage => this.getQueueMetrics(stage)));
        ...
        const bottleneckStage = stageMetrics.reduce((worst, current) => current.averageServiceTime > worst.averageServiceTime ? current : worst).stage;
