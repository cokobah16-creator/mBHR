...
class SupabaseSync {
    ...
    async syncPatients(direction = 'both') {
        if (!this.client)
        ...
        try {
            ...
            if (direction === 'upload' || direction === 'both') {
                const dirtyPatients = await db.patients.where('_dirty').equals(1).toArray();
            ...
            if (direction === 'download' || direction === 'both') {
                const lastSync = this.syncStatus.lastSync?.toISOString() || '1970-01-01';
    ...
    async setupRealtimeSync(table, callback) {
        if (!this.client)
            return null;
        return this.client
