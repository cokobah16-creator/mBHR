...
class SmartMedicationSystem {
  ...
  async checkInteractions(medications: string[]): Promise<MedicationInteraction[]> {
    ...
    for (let i = 0; i < medications.length; i++) {
      for (let j = i + 1; j < medications.length; j++) {
        ...
        if (drug1.interactions.some(int =>
          ...
          interactions.push(this.generateInteractionDetails(
            medications[i],
            medications[j],
            drug1,
            drug2
  ...
  async checkAllergies(
    ...
    for (const allergy of allergies) {
      ...
      if (medData.commonAllergies.some(a =>
        allergyName.includes(a) || a.includes(allergyName)
      )) {
        return {
          hasAllergy: true,
  ...
  async predictAdherence(patientId: string, medication: string): Promise<AdherencePrediction> {
    const patient = await db.patients.get(patientId)
    if (!patient) {
      throw new Error('Patient not found')
    }
