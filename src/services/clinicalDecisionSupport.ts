...
class ClinicalDecisionSupportService {
  ...
  async createClinicalAlert(alert: Omit<ClinicalAlert, 'id' | 'createdAt' | 'acknowledged'>): Promise<string> {
  ...
  async getPatientAlerts(patientId: string, unacknowledgedOnly: boolean = false): Promise<ClinicalAlert[]> {
  ...
  async generateHighRiskAlert(patientId: string, riskProfile: PatientRiskProfile): Promise<void> {
