...
describe("Predictive Queue - Wait Time Predictions", () => {
  ...
  it("should identify bottlenecks correctly", async () => {
    ...
    const hasBottleneck = predictions.some(
      (p) => p.bottleneckRisk === "high" || p.bottleneckRisk === "critical",
    );
...
describe("Predictive Queue - Queue Optimization", () => {
  ...
  it("should optimize queue order based on priority scores", async () => {
      ...
      equals: vi.fn(() => ({
        reverse: vi.fn(() => ({
          limit: vi.fn(() => ({
            toArray: vi
              .fn()
              .mockResolvedValue([{ tempC: 40.0, systolic: 185, spo2: 88 }]),
          })),
        })),
      })),
    });
    ...
    const optimizations =
      await predictiveQueue.optimizeQueueOrder("registration");
...
describe("Predictive Queue - Report Generation", () => {
  ...
  it("should generate comprehensive queue reports", async () => {
    ...
    const report = await predictiveQueue.generateQueueReport(
      startDate,
      endDate,
    );
