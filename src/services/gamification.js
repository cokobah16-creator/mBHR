...
export class GamificationService {
    ...
    static calculateTokens(baseTokens, multipliers = {}) {
        const { streak = 0, accuracy = 1.0, teamBonus = 0, speedBonus = 1.0, qualityBonus = 1.0 } = multipliers;
        ...
        const finalTokens = Math.round(baseTokens * streakMult * accMult * speedBonus * qualityBonus) + teamBonus;
        return Math.max(1, finalTokens); // Minimum 1 token
    }
    // Start a new game session
    static async startSession(type, volunteerId, payload = {}) {
        const session = {
            id: generateId(),
            type,
            volunteerId,
    ...
    static async completeSession(sessionId, result) {
    ...
    static async approveSession(sessionId, approverId) {
    ...
    static async updateWallet(volunteerId, tokensEarned, maintainStreak = true) {
        const now = new Date();
    ...
    // Get leaderboard
    static async getLeaderboard(period = 'all', limit = 20) {
        try {
    ...
    static async checkBadgeEligibility(volunteerId, gameType) {
        const newBadges = [];
        try {
