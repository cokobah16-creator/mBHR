 ...
 */
export async function processPortalInvitationQueue() {
    if (isProcessing) {
 ...
 */
export async function syncPortalActivityFromSupabase() {
    let synced = 0;
    let errors = 0;
    try {
        // Check if online
        ...
        try {
            ...
            for (const supabasePatient of supabasePatients || []) {
                try {
                    ...
                    const hasUpdates = supabasePatient.auth_uid !== localPatient.authUid ||
 ...
 */
export function startPortalSyncWorker(intervalSeconds = 30) {
 ...
 */
export function stopPortalSyncWorker() {
    if (syncIntervalId !== null) {
