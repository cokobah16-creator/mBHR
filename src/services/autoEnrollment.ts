    ...
    for (const row of data || []) {
      switch (row.setting_key) {
        case 'auto_enrollment_enabled':
          settings.autoEnrollmentEnabled = row.setting_value === true || row.setting_value === 'true'
          break
        case 'require_email':
          settings.requireEmail = row.setting_value === true || row.setting_value === 'true'
          break
        case 'send_welcome_notification':
          settings.sendWelcomeNotification = row.setting_value === true || row.setting_value === 'true'
          break
      }
    }
      ...
      if (settings.sendWelcomeNotification) {
        await supabase
          ...
          .insert({
            ...
            message: 'Your patient portal account has been activated. You can now view your health records, request appointments, and message your care team.',
            priority: 'normal'
          })
      }
    }
...
export async function bulkAutoEnroll(
  ...
  try {
    ...
    logger.log(`Bulk auto-enrollment complete: ${enrolled} enrolled, ${skipped} skipped, ${failed} failed`)
  } catch (error) {
