...
async function sendSMS(to: string, message: string, reminderId?: string): Promise<SMSResult> {
...
async function processOutboundMessage(msg: OutboundMessage): Promise<boolean> {
  const result = await sendSMS(msg.to, msg.payload.message as string || '', msg.id);
  ...
  if (result.success) {
    ...
    if (msg.payload.reminderId) {
      await markReminderSent(msg.payload.reminderId as string).catch(console.warn);
    }
...
async function runProcessingCycle(): Promise<{ reminders: number; messages: number }> {
  ...
  try {
    ...
    if (reminders > 0 || messages > 0) {
      console.log(`Notification worker: processed ${reminders} reminders, ${messages} messages`);
    }
...
export async function processNow(): Promise<{ reminders: number; messages: number }> {
