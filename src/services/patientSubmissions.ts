...
export async function createPatientSubmission(
  ...
  try {
    ...
    if (supabase) {
      const { error } = await supabase.from("patient_submitted_data").insert({
        id: submission.id,
...
export async function getPendingSubmissionsForReview(): Promise<
  ...
  try {
    if (supabase) {
      ...
      if (error) {
        ...
        return await db.patientSubmittedData
          .where("status")
          .equals("pending")
          .toArray();
      }
...
export async function mergeSubmissionToAllergies(
  ...
  try {
    ...
    const allergiesData = submission.data as {
...
export async function syncPendingSubmissions(): Promise<void> {
  ...
  try {
    ...
    for (const submission of dirtySubmissions) {
      const { error } = await supabase.from("patient_submitted_data").upsert({
        id: submission.id,
        patient_id: submission.patientId,
        portal_user_id: submission.portalUserId,
        submission_type: submission.submissionType,
