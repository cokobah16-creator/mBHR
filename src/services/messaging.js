...
export class MessageService {
    ...
    async queueMedicationReminder(patientId, medicationName, dosage, frequency, scheduledFor) {
    ...
    async queueAppointmentReminder(patientId, appointmentDate, scheduledFor) {
        const patient = await db.patients.get(patientId);
        if (!patient)
            throw new Error('Patient not found');
        return MessageQueue.queueMessage(patientId, patient.phone, 'appointment.reminder', {
            patientName: `${patient.givenName} ${patient.familyName}`,
...
export function configureMessageService(gateway) {
    messageService = new MessageService(gateway);
}
