...
export async function syncNow() {
    if (!supabase)
        return console.log('[sync] supabase client not set — skipped');
    ...
    if (changed.length) {
        const { error } = await supabase.from('inventory_nm').upsert(changed.map(x => ({
            id: x.id,
        ...
        if (error)
            console.error('[sync] push inventory_nm failed', error);
    }
