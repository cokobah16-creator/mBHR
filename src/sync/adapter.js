...
export async function pushChanges() {
    ...
    for (const t of tables) {
        ...
        if (!dirty?.length)
            continue;
        const payload = dirty.map((r) => toDB(r, mapToDB[t]));
        console.log('SYNC push payload', t, payload[0]);
        const { error } = await sb.from(t).upsert(payload, { onConflict: 'id' });
        if (error) {
            console.warn('push error', t, error);
            continue;
        }
        await db[t].where('_dirty').equals(1).modify({ _dirty: 0, _syncedAt: new Date().toISOString() });
    }
}
