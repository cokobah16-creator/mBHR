...
import { db } from '../db';
import { processQueue } from '../stores/operationsQueue';
...
const mapFromDB = Object.fromEntries(Object.entries(mapToDB).map(([t, m]) => [t, Object.fromEntries(Object.entries(m).map(([app, db]) => [db, app]))]));
function toDB(obj, map) {
    const out = {};
    for (const [appKey, dbKey] of Object.entries(map))
        if (obj[appKey] !== undefined)
            out[dbKey] = obj[appKey];
    return out;
}
function fromDB(obj, map) {
    ...
    for (const [dbKey, appKey] of Object.entries(map))
        if (obj[dbKey] !== undefined)
            out[appKey] = obj[dbKey];
    return out;
}
const tables = ['app_users', 'patients', 'visits', 'vitals', 'consultations', 'dispenses', 'inventory', 'queue', 'patient_allergies', 'patient_preferences'];
...
const CURSOR_KEY = (t) => `sync_cursor:${t}`;
...
async function setCursor(table, ts) {
    ...
    await db.settings.put({ key: CURSOR_KEY(table), value: iso });
}
async function detectConflict(table, id, localData) {
    if (!sb)
...
export async function pushChanges() {
    ...
    for (const t of tables) {
        ...
        const dirty = await db[localTable].where('_dirty').equals(1).toArray().catch(() => []);
        if (!dirty?.length)
            continue;
        for (const record of dirty) {
            ...
            if (!error) {
                await db[localTable].update(record.id, {
                    _dirty: 0,
...
export async function pullChanges() {
    ...
    for (const t of tables) {
        ...
        for (const row of data ?? []) {
            ...
            await db[localTable].put({ ...mapped, _dirty: 0, _syncedAt: new Date().toISOString() });
...
export async function processOperationsQueue() {
    const detectedConflicts = [];
    await processQueue(async (operation) => {
        ...
        // Check for conflicts
        const conflictCheck = await detectConflict(table, operation.entityId, operation.data);
        if (conflictCheck.hasConflict && conflictCheck.conflicts) {
            detectedConflicts.push({
                entityType: table,
                entityId: operation.entityId,
                localTimestamp: operation.data.updatedAt || new Date().toISOString(),
                remoteTimestamp: conflictCheck.remoteData.updated_at,
                conflicts: conflictCheck.conflicts
            });
        ...
        if (operation.type === 'create' || operation.type === 'update') {
            const payload = toDB(operation.data, mapToDB[table]);
            const { error } = await sb.from(table).upsert(payload, { onConflict: 'id' });
            if (error)
                throw error;
            // Mark as synced in local DB
            const localTable = localTableMap[table];
            await db[localTable].update(operation.entityId, {
                _dirty: 0,
                _syncedAt: new Date().toISOString()
            });
        }
