...
describe('Conflict Resolver', () => {
    ...
    describe('keep-local strategy', () => {
        it('should mark local record as dirty to force sync', async () => {
            ...
            expect(db.patients.update).toHaveBeenCalledWith('patient-123', expect.objectContaining({
                _dirty: 1,
                _syncedAt: null
            }));
        });
    ...
    describe('keep-remote strategy', () => {
        it('should update local record with remote data', async () => {
            ...
            await resolveConflict(mockConflict, 'keep-remote', undefined, undefined, remoteData);
            expect(db.patients.put).toHaveBeenCalledWith(expect.objectContaining({
                id: 'patient-123',
                givenName: 'Jonathan',
                phone: '08087654321',
                _dirty: 0,
    ...
    describe('manual strategy', () => {
        it('should merge fields based on manual resolution', async () => {
            ...
            await resolveConflict(mockConflict, 'manual', resolution, localData, remoteData);
            expect(db.patients.put).toHaveBeenCalledWith(expect.objectContaining({
                id: 'patient-123',
                givenName: 'John',
                phone: '08087654321',
        ...
        it('should throw error if local or remote data missing', async () => {
            await expect(resolveConflict(mockConflict, 'manual', {})).rejects.toThrow('Both local and remote data required');
        });
    });
