...
type GamState = {
  ...
  leaderboard: (
    period?: Period,
    limit?: number,
  ) => Promise<Array<{ volunteerId: string; tokens: number }>>;
};

export const useGam = create<GamState>((set, _get) => ({
  wallet: 0,
  loading: false,
  ...
  addTokens: async (volunteerId, amount, badge) => {
    ...
    await mbhrDb.gamification.update(volunteerId, {
      tokens: next,
  ...
  leaderboard: async (_period = "all", limit = 20) => {
