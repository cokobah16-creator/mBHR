  ...
  db: {
    patients: {
      orderBy: vi.fn(() => ({
        reverse: vi.fn(() => ({
          toArray: vi.fn(() =>
            Promise.resolve(Array.from(mockPatients.values())),
          ),
        })),
      })),
...
describe("usePatientsStore", () => {
  ...
  describe("startVisit", () => {
    it("should create a new visit", async () => {
      const visitId = await usePatientsStore
        .getState()
        .startVisit("patient-1", "Test Clinic");
  ...
  describe("updatePatient", () => {
    it("should update patient in database", async () => {
      ...
      await usePatientsStore
        .getState()
        .updatePatient("p1", { phone: "08099999999" });
