...
export function useAudioPrompts() {
  ...
  const speakWithTTS = useCallback((text: string, lang: string) => {
  ...
  const playPrompt = useCallback(async (key: string, fallbackText?: string) => {
  ...
  }, [settings.enabled, settings.preferTTS, isPlaying, locale, speakWithTTS, playAudioFile])

  const preloadAudio = useCallback((keys: string[]) => {
    keys.forEach(key => {
