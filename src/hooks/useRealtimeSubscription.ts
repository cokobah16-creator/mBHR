...
interface UseRealtimeOptions {
  ...
  onUpdate?: (data: Record<string, unknown>, old: Record<string, unknown>) => void
...
export function useRealtimeSubscription(
  ...
  useEffect(() => {
    ...
    switch (type) {
      ...
      case 'consultations':
        unsubscribe = patientId
          ? realtimeSyncService.subscribeToConsultations(patientId, handlePayload)
          : realtimeSyncService.subscribeToAllConsultations(handlePayload)
        break
      case 'appointments':
        unsubscribe = patientId
          ? realtimeSyncService.subscribeToAppointments(patientId, handlePayload)
          : realtimeSyncService.subscribeToAllAppointments(handlePayload)
        break
      case 'messages':
        if (patientId) {
          unsubscribe = realtimeSyncService.subscribeToMessages(patientId, handlePayload)
        }
        break
      case 'notifications':
        if (patientId) {
          unsubscribe = realtimeSyncService.subscribeToNotifications(patientId, handlePayload)
        }
        break
      case 'queue':
        unsubscribe = realtimeSyncService.subscribeToQueue(stage, handlePayload)
        break
      case 'submissions':
        unsubscribe = realtimeSyncService.subscribeToPatientSubmissions(handlePayload)
        break
    }
...
export function useVitalsRealtime(patientId: string, onChange: () => void): void {
  useRealtimeSubscription('vitals', { patientId, onChange })
}

export function useConsultationsRealtime(patientId: string, onChange: () => void): void {
  useRealtimeSubscription('consultations', { patientId, onChange })
}

export function useAppointmentsRealtime(patientId: string, onChange: () => void): void {
  useRealtimeSubscription('appointments', { patientId, onChange })
}

export function useMessagesRealtime(patientId: string, onChange: () => void): void {
