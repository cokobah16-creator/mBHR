import { db, VitalsRange, generateId } from './index'
...
export async function seedVitalsRanges(): Promise<void> {
  try {
    ...
    const ranges: VitalsRange[] = VITALS_RANGES.map(data => ({
      id: generateId(),
      ageMin: data.ageMin,
      ageMax: data.ageMax,
      sex: data.sex,
...
export function getVitalRange(
  ...
  sex: 'M' | 'F' | 'U',
  metric: 'hr' | 'rr' | 'temp' | 'sbp' | 'dbp' | 'spo2'
): Promise<VitalsRange | undefined> {
  return db.vitalsRanges
    .where('metric').equals(metric)
    .and(range =>
      ageYears >= range.ageMin &&
...
export async function checkVitalInRange(
  ageYears: number,
  sex: 'M' | 'F' | 'U',
  metric: 'hr' | 'rr' | 'temp' | 'sbp' | 'dbp' | 'spo2',
  value: number
...
export async function flagAbnormalVitals(
  ageYears: number,
  sex: 'M' | 'F' | 'U',
  vitals: {
    pulseBpm?: number
  ...
  if (vitals.pulseBpm) {
    ...
    if (hr.low) flags.push('Low heart rate (bradycardia)')
    if (hr.high) flags.push('High heart rate (tachycardia)')
  }

  if (vitals.tempC) {
    const temp = await checkVitalInRange(ageYears, sex, 'temp', vitals.tempC)
    if (temp.low) flags.push('Low temperature (hypothermia)')
    if (temp.high) flags.push('High temperature (fever)')
  }

  if (vitals.systolic) {
    const sbp = await checkVitalInRange(ageYears, sex, 'sbp', vitals.systolic)
    if (sbp.low) flags.push('Low systolic BP (hypotension)')
    if (sbp.high) flags.push('High systolic BP (hypertension)')
  }

  if (vitals.diastolic) {
    const dbp = await checkVitalInRange(ageYears, sex, 'dbp', vitals.diastolic)
    if (dbp.low) flags.push('Low diastolic BP')
    if (dbp.high) flags.push('High diastolic BP')
  }

  if (vitals.spo2) {
    const spo2 = await checkVitalInRange(ageYears, sex, 'spo2', vitals.spo2)
    if (spo2.low) flags.push('Low oxygen saturation (hypoxia)')
  }

  return flags
}

export async function getVitalsInterpretation(
  ageYears: number,
  sex: 'M' | 'F' | 'U',
  vitals: {
    pulseBpm?: number
