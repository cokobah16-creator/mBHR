...
export class GamificationDB extends Dexie {
  ...
  constructor() {
    ...
    this.version(1).stores({
      ...
      game_attempts:
        "id, taskCode, actorId, startedAt, finishedAt, status, _dirty, _syncedAt",
...
export function calculateTokens(
  ...
  const finalTokens =
    Math.round(baseTokens * streakMult * accMult * speedBonus * qualityBonus) +
    teamBonus;
...
export async function checkBadgeEligibility(
  ...
  try {
    const attempts = await gamificationDb.game_attempts
      ...
      .and(
        (attempt) =>
          attempt.status === "verified" || attempt.status === "completed",
      )
      .toArray();
    ...
    const _totalAttempts = attempts.length;
    ...
    if (
      wallet?.streakDays &&
      wallet.streakDays >= 7 &&
      !existingBadgeCodes.has("week_warrior")
    ) {
    ...
    if (taskCode === "queue_maestro") {
      const queueAttempts = attempts.filter(
        (a) => a.taskCode === "queue_maestro",
      );
      if (
        queueAttempts.length >= 50 &&
        !existingBadgeCodes.has("flow_master")
      ) {
