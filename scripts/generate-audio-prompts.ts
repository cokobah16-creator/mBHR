...
function printSummary() {
  ...
  console.log(
    "- Patient registration (5): givenName, familyName, phone, dob, address",
  );
  console.log(
    "- Vitals recording (4): height, weight, temperature, bloodPressure",
  );
...
async function main() {
  ...
  if (args.includes("--script")) {
    ...
    console.log("\nUsage:");
    console.log(
      "  npx tsx scripts/generate-audio-prompts.ts --script      # Print recording script",
    );
    console.log(
      "  npx tsx scripts/generate-audio-prompts.ts --placeholders # Create placeholder files",
    );
  }
}
