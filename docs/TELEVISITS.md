...
1. Doctors, nurses, and admins open **Televisits** (`/televisits`) from the sidebar. The Doctor Dashboard also shows a **Televisits** button with a badge counting pending requests. Scheduling requires the signed-in staff member to exist in the online staff directory (`app_users`, whose ids are Supabase auth uuids and are foreign keys for `appointments.provider_id` / `created_by`). The app resolves that id from the current Supabase session first (online sign-in keeps the local ULID user record, so the session id is the one `app_users` knows) and falls back to the local id. If neither is registered, the page shows a notice and scheduling is refused with a clear message: sign in online, or have an admin add the account to the staff directory.
...
3. Scheduling rejects times already in the past and slots that overlap another active appointment of the same provider. It then claims the request (only while it is still `pending`, so a concurrent scheduler or a patient who has just cancelled gets a clear error instead of a duplicate visit), inserts an `appointments` row (`status = 'scheduled'`, `visit_mode = 'televisit'`, fresh `meeting_link`), and links the appointment id back to the request; if the insert fails the request is released to `pending` again. The patient's phone (local Dexie record first, then Supabase) is used to send the `televisit_scheduled` SMS. If the SMS fails the visit is still scheduled and a toast reports the failure so staff can share the link another way.
...
Migration: `supabase/migrations/20260910161049_add_televisits.sql`.
...
Three migrations make up the feature. They were applied to the production project through the Supabase connector and are recorded in its migration history under the same version numbers as the files:

- `20260910161049_add_televisits.sql` — columns, indexes, RLS, SMS template seeds
- `20260910164216_drop_legacy_appointment_write_policies.sql` — retires the overlapping role-list write policies
- `20260910164515_wrap_televisit_policy_jwt_claims.sql` — wraps the JWT claim read in the new policies so it is evaluated once per statement

For another environment:

```bash
supabase migration list                       # compare local files with the remote history first
supabase db push                              # applies any of the three not yet recorded remotely
...
`supabase db push` only skips files whose version number is already in the remote history, so if a migration was applied under a different version (for example through the dashboard or the connector), mark it with `supabase migration repair --status applied <version>` rather than letting it re-run. All three files are idempotent, so an accidental re-run is harmless.
