...
| 44 | Portal access can be switched off at registration | On the full registration form, unticking 'Enable patient portal access' now works. Before, the box could not really be unticked, so the patient was enrolled anyway. Typing a phone number or email no longer re-ticks it once staff have made their own choice. Portal enrolment happens only when the box is ticked, and the 'terms explained and agreed' tick is still required then. No threshold, range or dose changed. | `components/PatientForm.tsx` | Not recommended |
...
### 2.5 Audit follow-up (September 2026): questions for clinical review
