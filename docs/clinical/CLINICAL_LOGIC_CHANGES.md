...
- [x] **Staff get no documents through the connection.** No staff account
      ...
      documents and download only files they uploaded. Document uploads
      get no provenance either: staff with audit access no longer see
      through the connection's provenance records that a document was
      uploaded (its id, when, or whether through the portal). Still open:
      the access log (AuditEvent) is a separate open owner decision in
      `docs/interoperability/security.md` and is unchanged. With patient
      access switched on, it shows staff with audit access when a patient
      opened one of their documents or files (the document's id and the
      time) or listed their documents (how many were returned). A patient
      downloads only files they uploaded, so a download in the log shows
      that they uploaded that document.
      Decision received 2026-09-26: **patients only** (sheet 11, as
      recommended). Built: see section 4.
      Decision received 2026-09-26: **no provenance for document uploads**
      (Emeke Okobah, asked after the sign-off and not on the sheet; as
      recommended). Built: see section 4.
...
and 13 needed building and are built in pull request #156 (section 4,
"FHIR sign-off: approved changes"); decisions 1, 4, 8, 9 and 10
