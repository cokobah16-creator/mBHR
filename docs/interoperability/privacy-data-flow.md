...
| R3 | Production database drift (F3): row-level security or helpers differ from the repository, so reads return more than intended | Medium | High | the gateway checks role and permission in the database itself and fails closed when helpers are missing; do not enable before reconciliation | HRIS reconciliation work, owner go-ahead |
