...
| # | Blocker | Why |
| --- | --- | --- |
| B1 | Findings F1 to F4 in [security.md](security.md#findings) resolved, and the TEFCA functions (`tefca-oauth`, `tefca-ias`, `tefca-bulk`) undeployed or rebuilt on this gateway | `tefca-oauth` has its own `/.well-known/smart-configuration` and token endpoint backed by the service role (its deployed copy ran older code, F1, and was deleted on 26 September 2026); two authorization servers for the same data cannot both be governed |
