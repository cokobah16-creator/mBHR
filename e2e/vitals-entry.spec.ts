...
test.describe("Vitals Entry Flow", () => {
  ...
  test("should navigate to vitals page and show patient search", async ({
    page,
  }) => {
  ...
  test("should display vitals form after patient selection", async ({
    page,
  }) => {
    ...
    await expect(page.locator("#vitals-heightCm")).toBeVisible({
      timeout: 10000,
    });
  ...
  test("should calculate BMI when height and weight are entered", async ({
    page,
  }) => {
  ...
  test("should submit vitals form and navigate to consult", async ({
    page,
  }) => {
