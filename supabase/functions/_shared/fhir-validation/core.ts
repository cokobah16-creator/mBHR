// Portable US Core 7.0 validator. Single source of truth for both the
// browser bundle (re-exported via src/services/fhir/uscore-validator.ts and
// src/services/fhir/validation/core.ts) and the Supabase edge functions
// (imported directly from supabase/functions/tefca-ias/validation.ts).
//
// Phase D-3 relocation: this validator previously lived under
// src/services/fhir/validation/core.ts and was imported across the
// src/ -> supabase/ boundary by the Deno edge function, which left the
// runtime path conditional on Supabase Edge bundling following arbitrary
// upward imports. After D-3 the Deno-side import is canonical (no cross
// boundary), and the browser side crosses the boundary in the cheaper
// direction via thin re-export shims that Vite + tsc both resolve cleanly.
//
...
//   - Imports only from sibling pure-type modules.
//
