...
| 20260125091822 | add_immunizations_conditions_sdoh | **C5.** Delete the six policy DO blocks (L123-238; they fail with uuid = text and give volunteers/nurses DELETE); add permission-matrix policies instead. Pin `search_path` on `update_immunizations_updated_at()`. REVOKE anon. **Owner decision 2.7 (2026-09-26):** `clinical_status`, `verification_status` and `category` on `conditions` have no default (removed in this file, which also drops them from an older copy of the table); do not add one back. Checked by `scripts/ci/conditions-no-defaults.test.ts`. |
