...
- Spar Nigeria — upscale, targets premium buyers
...
- Schools and universities (canteens, tuck shops) — UNIABUJA, ABUAD
...
- FMCG distributors in Abuja who already serve supermarkets
- Area distributors in Kubwa, Nyanya, Lugbe who supply smaller shops

Always call the retailer_discovery function to return your structured output. Fit scores (1–10) should reflect channel reach, alignment with snack impulse-buying behaviour, and Abuja-specific logistics feasibility.`;

const retailerDiscoveryTool = {
  ...
  parameters: {
    ...
    properties: {
      summary: {
        type: "string",
        description: "Strategic overview of the best distribution approach in Abuja (3–5 sentences).",
      },
      leads: {
        type: "array",
        ...
        items: {
          ...
          properties: {
            ...
            type: {
              type: "string",
              description:
                "One of: supermarket, filling_station_shop, open_market, convenience_store, institutional, online_platform, distributor, hotel_hospitality",
            },
            abujaArea: {
              type: "string",
            ...
            contactStrategy: {
              type: "string",
              description: "Concrete, actionable outreach steps for this channel (2–4 sentences)",
            },
            reasoning: {
              type: "string",
            ...
            estimatedMonthlyVolume: {
              type: "string",
              description: "Rough units or packs per month e.g. 500–1,000 units/month",
            },
            minOrderNote: {
              type: "string",
              description: "Any known minimum order or shelf-listing requirement",
            },
          },
      ...
      quickWins: {
        type: "array",
        items: { type: "string" },
        description: "2–3 immediate first steps Fine Boy Foods should take this week",
      },
    },
...
app.post("/api/discover", async (req, res) => {
  ...
  if (!apiKey) {
    return res.status(500).json({ error: "GOOGLE_API_KEY is not set in .env file." });
  }
  ...
  const model = genAI.getGenerativeModel({
    ...
    tools: [{ functionDeclarations: [retailerDiscoveryTool] }],
...
Identify 6–10 specific, realistic retail and distribution leads in Abuja, FCT that Fine Boy Foods should approach. Prioritise channels that suit packaged snack impulse-buying and have accessible supplier onboarding for a growing Nigerian food brand.`;
  ...
  try {
    ...
    const fnCall = parts.find((p) => p.functionCall?.name === "retailer_discovery");

    if (!fnCall) {
      return res.status(500).json({ error: "Agent did not return structured results. Try again." });
    }

    res.json({ success: true, result: fnCall.functionCall.args });
  } catch (err) {
